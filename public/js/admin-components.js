              ...
              <div class="database-details">
                ...
                <div class="db-status-list">
                  <div class="db-status-item">
                    <div class="db-info">
                      <span class="db-name">Cloudflare D1</span>
                      <span class="db-status" :class="databaseStatus.connected ? 'status-success' : 'status-error'">
                        {{ databaseStatus.connected ? '已连接' : '连接失败' }}
                      </span>
                    </div>
                    <div class="db-description">Cloudflare 边缘 SQLite 数据库</div>
            ...
            <div class="settings-card">
              ...
              <div v-else-if="databaseTables.data" class="tables-info">
                <div class="tables-summary">
                  <p><strong>数据库类型:</strong> Cloudflare D1</p>
    ...
    const loadDatabaseStatus = async () => {
      try {
        ...
        if (data.success) {
          ...
          databaseStatus.value.error = data.message;
        }
      } catch (error) {
        console.error('加载数据库状态失败:', error);
        databaseStatus.value.connected = false;
        databaseStatus.value.error = error.message;
      } finally {
