...
const AdminView = {
  template: `
            ...
            <div class="settings-card">
              ...
              <div class="form-group">
                ...
                <div class="database-status">
                  <span class="current-db" :class="databaseSettings.current_type">
                    {{ getDatabaseTypeName(databaseSettings.current_type) }}
                  </span>
                  <button @click="loadDatabaseStatus" class="btn btn-sm btn-secondary" :disabled="databaseLoading">
                    <i class="fas fa-sync-alt" :class="{ 'fa-spin': databaseLoading }"></i>
                    刷新状态
                  </button>
                </div>
              </div>
              ...
              <div class="form-group">
                <label>切换数据库</label>
                <select v-model="selectedDatabaseType" :disabled="databaseLoading">
                  <option v-for="db in availableDatabases" :key="db.type" :value="db.type" :disabled="!db.available">
                    {{ db.name }} {{ db.available ? (db.connected ? '✅' : '⚠️') : '❌' }}
                  </option>
                </select>
                <small class="help-text">
                  选择要使用的数据库类型。切换前请确保目标数据库已正确配置。
                </small>
              </div>

              <div class="database-actions">
                <button @click="switchDatabase" class="btn btn-warning"
                        :disabled="databaseLoading || selectedDatabaseType === databaseSettings.current_type">
                  <i class="fas fa-exchange-alt"></i>
                  切换数据库
                </button>
                <button @click="showMigrationDialog = true" class="btn btn-info" :disabled="databaseLoading">
                  <i class="fas fa-arrow-right"></i>
                  数据迁移
                </button>
              </div>
              ...
              <!-- 数据库状态详情 -->
              <div class="database-details" v-if="availableDatabases.length > 0">
                <h4>数据库状态</h4>
                <div class="db-status-list">
                  <div v-for="db in availableDatabases" :key="db.type" class="db-status-item">
                    <div class="db-info">
                      <span class="db-name">{{ db.name }}</span>
                      <span class="db-status" :class="getStatusClass(db)">
                        {{ getStatusText(db) }}
                      </span>
                    </div>
    ...
    <!-- 数据迁移对话框 -->
    <div v-if="showMigrationDialog" class="modal-overlay" @click="showMigrationDialog = false">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3><i class="fas fa-arrow-right"></i> 数据库迁移</h3>
          <button @click="showMigrationDialog = false" class="close-btn">&times;</button>
        </div>
        <div class="modal-body">
          <div class="migration-form">
            <div class="form-group">
              <label>源数据库</label>
              <select v-model="migrationSettings.from_type">
                <option v-for="db in availableDatabases" :key="db.type" :value="db.type" :disabled="!db.connected">
                  {{ db.name }} {{ db.connected ? '✅' : '❌' }}
                </option>
              </select>
            </div>
            ...
            <div class="form-group">
              <label>目标数据库</label>
              <select v-model="migrationSettings.to_type">
                <option v-for="db in availableDatabases" :key="db.type" :value="db.type" :disabled="!db.connected">
                  {{ db.name }} {{ db.connected ? '✅' : '❌' }}
                </option>
              </select>
            ...
            <div class="form-group">
              ...
              <div class="table-selection">
                <label class="checkbox-item">
                  <input type="checkbox" v-model="migrationSettings.selectAll" @change="toggleAllTables">
                  全选
                </label>
                <label v-for="table in migrationTables" :key="table" class="checkbox-item">
                  <input type="checkbox" v-model="migrationSettings.tables" :value="table">
                  {{ table }}
                </label>
              </div>
            </div>
            ...
            <div class="migration-warning">
              ...
              <strong>注意：</strong>数据迁移会覆盖目标数据库中的现有数据。请确保已备份重要数据。
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <button @click="showMigrationDialog = false" class="btn btn-secondary">取消</button>
          <button @click="startMigration" class="btn btn-danger" :disabled="migrationLoading || !canStartMigration">
            <i class="fas fa-arrow-right" :class="{ 'fa-spin': migrationLoading }"></i>
            {{ migrationLoading ? '迁移中...' : '开始迁移' }}
          </button>
        </div>
      </div>
    </div>
  ...
  setup() {
    ...
    const migrationTables = ['bookmarks', 'categories', 'settings', 'users', 'sessions', 'sync_logs'];
    ...
    const canStartMigration = computed(() => {
      return migrationSettings.value.from_type !== migrationSettings.value.to_type &&
             migrationSettings.value.tables.length > 0;
    });
    ...
    const getStatusClass = (db) => {
      if (!db.available) return 'status-unavailable';
      if (!db.connected) return 'status-error';
      return 'status-success';
    };

    const getStatusText = (db) => {
      if (!db.available) return '未配置';
      if (!db.connected) return '连接失败';
      return '已连接';
    };
    ...
    const loadDatabaseStatus = async () => {
      try {
        ...
        if (data.success) {
          databaseSettings.value.current_type = data.current_type;
          availableDatabases.value = data.available_types || [];
          selectedDatabaseType.value = data.current_type;
        } else {
    ...
    const switchDatabase = async () => {
      if (selectedDatabaseType.value === databaseSettings.value.current_type) {
        alert('已经是当前数据库类型');
        return;
      }

      const targetDb = availableDatabases.value.find(db => db.type === selectedDatabaseType.value);
      if (!targetDb || !targetDb.connected) {
        alert('目标数据库未连接，无法切换');
        return;
      }

      if (!confirm(`确定要切换到 ${targetDb.name} 数据库吗？\n\n切换后将使用新的数据库，请确保数据已正确迁移。`)) {
        return;
      }
      ...
      try {
        ...
        const response = await fetch('/api/database/switch', {
          ...
          body: JSON.stringify({
            database_type: selectedDatabaseType.value
          })
        });
        ...
        if (data.success) {
          alert(`成功切换到 ${targetDb.name} 数据库`);
          databaseSettings.value.current_type = data.new_type;
    ...
    const toggleAllTables = () => {
      if (migrationSettings.value.selectAll) {
        migrationSettings.value.tables = [...migrationTables];
      } else {
        migrationSettings.value.tables = [];
      }
    };
    ...
    const startMigration = async () => {
      ...
      const fromDb = availableDatabases.value.find(db => db.type === migrationSettings.value.from_type);
      const toDb = availableDatabases.value.find(db => db.type === migrationSettings.value.to_type);
      ...
      const confirmMessage = `确定要从 ${fromDb.name} 迁移数据到 ${toDb.name} 吗？\n\n` +
                           `迁移表: ${migrationSettings.value.tables.join(', ')}\n\n` +
                           `⚠️ 警告: 这将覆盖目标数据库中的现有数据！`;
      ...
      try {
        ...
        const response = await fetch('/api/database/migrate', {
          ...
          body: JSON.stringify({
            from_type: migrationSettings.value.from_type,
            to_type: migrationSettings.value.to_type,
            tables: migrationSettings.value.tables
          })
        });
        ...
        if (data.success) {
          ...
          let message = `数据迁移完成！\n\n`;
          message += `成功迁移: ${summary.successful_tables || 0} 个表\n`;
          message += `失败: ${summary.failed_tables || 0} 个表\n`;
          message += `总行数: ${summary.total_rows || 0} 行\n\n`;

          if (data.results && data.results.length > 0) {
            message += '详细结果:\n';
            data.results.forEach(result => {
              const status = result.success ? '✅' : '❌';
              message += `${status} ${result.table}: ${result.message || result.error}\n`;
            });
          }
